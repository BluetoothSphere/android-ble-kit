...
fun ByteArray.getULong(offset: UInt = 0u, length: UInt, order: ByteOrder): ULong {
    ...
    val range: IntProgression =
        if (order == LITTLE_ENDIAN) IntProgression.fromClosedRange(end, start, -1) else start..end
    var result: ULong = 0u
    for (i in range) {
        if (i != range.first) {
            result = result shl 8
...
fun byteArrayOf(value: ULong, length: UInt, order: ByteOrder = LITTLE_ENDIAN): ByteArray {
    ...
    val range: IntProgression =
        if (order == LITTLE_ENDIAN) 0..end else IntProgression.fromClosedRange(end, 0, -1)
    for (i in range) {
        if (i == range.first) {
